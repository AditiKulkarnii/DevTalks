...
export const searchBlogsCount = (req, res) => {
    try {
        ...
        if (tag) {
            // Ensure the tag is trimmed and converted to lowercase
            findQuery = { tags: tag.trim().toLowerCase(), draft: false };
        } else if (query) {
            // Convert the query to lowercase for consistent search
            findQuery = {
                draft: false,
                $or: [
                    { title: new RegExp(query, 'i') },
        ...
        console.log("searchBlogsCount findQuery:", findQuery);  // Debug log

        Blog.countDocuments(findQuery)
        .then(count => {
            console.log("Count result:", count);  // Debug log
            return res.status(200).json({ totalDocs: count });
        })
        .catch(err => {
            console.error("Error in countDocuments:", err.message);  // Error log
            return res.status(500).json({ error: err.message });
        });
    } catch (error) {
        console.error("Error in searchBlogsCount:", error.message);  // Error log
        return res.status(500).json({ error: error.message });
    }
};
...
// export const likeBlog = (req, res) => {
//     let user_id = req.user._id; // Assuming the user ID is stored in req.user._id
//     let { _id, isLikedByUser } = req.body;
//     let incrementVal = !isLikedByUser ? 1 : -1;
...
//             Notification.findOneAndDelete({ user: user_id, blog: _id, type: "like" })
//             .then(data => {
//                 return res.status(200).json({ liked_by_user: false, total_likes: blog.activity.total_likes });
//             })
//             .catch(err => {
//                 console.error(err);
